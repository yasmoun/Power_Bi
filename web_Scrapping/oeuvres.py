...
try:
            ...
            for oeuvre in all_oeuvres:
                ...
                if '•' in adre:
                    ...
                    if len(adre) >= 3: 
                        ...
                        if '-' in year:
                            index = year.index('-')
                            year_birth = year[index - 1]
                            year_death = year[index + 1]
                            year_birth=''.join(year_birth)
                            year_death=''.join(year_death)
                        else:
                            year_birth = ' '.join(year)
                            year_death="none"
                    else :
                        year_birth="none"
                        year_death="none"
                ...
                #sql = "INSERT INTO artists (nom, lieu, year_birth, year_death ,year_inscription) VALUES (%s,%s, %s, %s, %s)"
                #values = (artiste,lieu, year_birth, year_death,yearIn)
    ...
    cursor.close()
    conn.close()
    print("Interruption détectée. Fermeture forcée de la connexion...")